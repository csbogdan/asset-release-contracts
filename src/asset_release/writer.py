...
# A CONVENIENCE DEFAULT, not a dependency. This module is part of the release
# contract and must stay usable by a repository that has no `asset_discovery`
# tree at all — the ontology sidecar is exactly that, and its private copy of
# this file is what drifted and broke a customer install.
#
# So the path is resolved lazily from this file's location, which is correct
# inside the runtime repo and simply absent elsewhere. `read_schema_constants`
# already refuses loudly when the file is missing, which is the right answer
# for a caller that should have passed its own path.
...
def read_schema_constants(module_path: Path | None = None) -> dict[str, int]:
    ...
    for node in tree.body:
        # Annotated because the two branches disagree: `AnnAssign.target` is a
        # single Name|Attribute|Subscript, `Assign.targets` is a list[expr], and
        # without this mypy infers the narrower type from whichever branch it
        # sees first and rejects the other. It surfaced only when this file moved
        # from deploy/ into src/, where mypy actually runs.
...
#: Read from what the ARTIFACT actually contains, not from the Dockerfile.
#:
#: This said `{slot}/asset-ontology/serve-ontology`, taken from the image's
#: `CMD ["serve-ontology"]`. The shipped bundle contains no such path:
#: `build-ontology-artifact.sh` stages a PyInstaller bundle at
#: `dist/ontology-service/` whose launcher is `run-ontology.sh`, and that is what
#: assets_llm's own writer has always declared — which is why the ontology
#: currently running on a customer works.
#:
#: The mismatch was harmless only because this profile has never produced an
#: ontology manifest; assets_llm's private copy did. The moment that copy is
#: deleted and the ontology consumes this contract — the entire point of the
#: split — this value starts being used, and it would stamp a validly signed
#: manifest naming an executable that does not exist. The ontology would install,
#: verify, and never start: precisely the customer-facing failure this migration
#: exists to remove, reintroduced by the migration.
#:
#: It carries a ~107MB read-only `ontology.db` and migrates nothing, hence
#: `has_schema_window=False`.
_ONTOLOGY_PROFILE = ComponentProfile(
    typ="AD-ONTOLOGY-RELEASE",
    entrypoint=("{slot}/ontology-service/run-ontology.sh",),
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "path to the schema_version.py to read the four schema numbers from. "
            "Defaults to the copy next to this package inside the runtime repo. "
            "REQUIRED when this package is installed as a dependency: the default "
            "resolves relative to the install directory, where no asset_discovery "
            "source tree exists, so a `--component server` build would fail with a "
